...
- 成功后自动删除一次性任务；固定随机任务在晚间完成后生成下一组早晚配对时间。
...
- 取消该任务的补偿重试。
- 删除已完成的指定日期任务。
- 如果命中固定随机晚间时间，则生成下一组早晚配对时间；早间成功不会提前替换当天晚间时间。
- 重新安排剩余任务。
...
- 晚间时间在 `21:30-22:00` 之间随机。
- 晚间时间同时保证不早于早间计划时间后 `9 小时 30 分`。
- 早间任务成功后保留当天晚间任务，避免把今晚下班时间提前或错配到下一天。
- 晚间任务成功后再生成下一组早晚时间。
